import { Zap, Layout, Share2, BarChart } from 'lucide-react';

const features = [
    {
        name: 'Instant Preview',
        description: 'See exactly how your link will look on social media platforms in real-time as you type.',
        icon: Zap,
    },
    {
        name: 'Custom Metadata',
        description: 'Control the title, description, and image that appears when your link is shared.',
        icon: Layout,
    },
    {
        name: 'Optimized Assets',
        description: 'We automatically handle image resizing and format optimization for best performance.',
        icon: Share2,
    },
    {
        name: 'Analytics Ready',
        description: 'Track clicks and engagement on your shared links (coming soon).',
        icon: BarChart,
    },
];

export function Features() {
    return (
        <div className="py-24 sm:py-32 bg-muted/30">
            <div className="container mx-auto px-4">
                <div className="mx-auto max-w-2xl lg:text-center">
                    <h2 className="text-base font-semibold leading-7 text-primary">Deploy faster</h2>
                    <p className="mt-2 text-3xl font-bold tracking-tight text-foreground sm:text-4xl">
                        Everything you need to master your social presence
                    </p>
                    <p className="mt-6 text-lg leading-8 text-muted-foreground">
                        Stop relying on random scrapers. Take control of how your content appears across the web.
                    </p>
                </div>
                <div className="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-4xl">
                    <dl className="grid max-w-xl grid-cols-1 gap-x-8 gap-y-10 lg:max-w-none lg:grid-cols-2 lg:gap-y-16">
                        {features.map((feature) => (
                            <div key={feature.name} className="relative pl-16">
                                <dt className="text-base font-semibold leading-7 text-foreground">
                                    <div className="absolute left-0 top-0 flex h-10 w-10 items-center justify-center rounded-lg bg-primary">
                                        <feature.icon className="h-6 w-6 text-white" aria-hidden="true" />
                                    </div>
                                    {feature.name}
                                </dt>
                                <dd className="mt-2 text-base leading-7 text-muted-foreground">
                                    {feature.description}
                                </dd>
                            </div>
                        ))}
                    </dl>
                </div>
            </div>
        </div>
    );
}
