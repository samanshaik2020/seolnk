...
import {
    ArrowLeft, Globe, Lock, TrendingUp,
    Calendar, ExternalLink, Loader2, Activity, CheckCircle, Shield
} from 'lucide-react'
...
export default function ProtectedAnalyticsPage({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = use(params)
    const [loading, setLoading] = useState(true)
    const [link, setLink] = useState<ProtectedLinkData | null>(null)
            ...
            <header className="border-b bg-background/80 backdrop-blur-md sticky top-0 z-50">
                <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-bold tracking-tight">Protected Link Analytics</h1>
                            <p className="text-sm text-muted-foreground flex items-center gap-2">
                                <Lock className="h-3 w-3" /> {link.title || 'Protected Link'}
                            </p>
                        </div>
                    </div>
                ...
                <div className="max-w-7xl mx-auto space-y-8">
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <StatsCard
                            ...
                            value={totalUnlocks}
                            icon={CheckCircle}
                            subtitle="Correct password entries"
                        ...
                        <StatsCard
                            ...
                            icon={Shield}
                            subtitle="Link protection"
                            iconColor={link.is_active ? 'text-green-500' : 'text-destructive'}
                        />
                        <StatsCard
                            title="Created"
                                ...
                                {topReferrers.length === 0 ? (
                                    ...
                                    <div className="space-y-3">
                                        {topReferrers.map(([ref, count], i) => {
                                            ...
                                            const percentage = (count / Math.max(totalUnlocks, 1)) * 100
                                            return (
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Latest successful password entries</CardDescription>
                        </CardHeader>
                        <CardContent>
                            {recentActivity.length === 0 ? (
                                <p className="text-center text-muted-foreground py-8">
                                    No unlocks yet. Share your link to start tracking!
                                </p>
                            ) : (
